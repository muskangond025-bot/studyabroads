...
const AngledCard = ({
    ...
    return (
        ...
        >
            {/* The Image Card */}
            <div className="relative h-full w-full overflow-hidden border-2 border-[var(--color-champagne)] bg-black 
            min-h-[400px] shadow-2xl rounded-lg">
                <ImageWithFallback
                    src={item.url}
                ...
                <div className="absolute inset-0 bg-gradient-to-t from-black via-black/50 to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-300 flex flex-col justify-end p-6">
                    {item.date && (
                        <p className="text-[var(--color-champagne)] text-sm mb-2 tracking-wider uppercase">
                            {item.date}
                        </p>
                    )}
                    {item.title && (
                ...
                {/* Default title overlay (when not hovered) */}
                {item.title && (
                    <div className="absolute bottom-0 left-0 right-0 bg-gradient-to-t from-[var(--color-maroon)] to-transparent p-6 group-hover:opacity-0 transition-opacity duration-300">
                        <h3 className="text-white text-xl font-semibold">{item.title}</h3>
                        {item.date && (
                            <p className="text-[var(--color-champagne)] text-sm mt-1">
                                {item.date}
                            </p>
                        )}
                    </div>
...
export const AngledSlider = ({
    ...
    useEffect(() => {
        const calculateWidth = () => {
            const numWidth = parseInt(cardWidth?.toString().replace("px", "") || "400");
    ...
    return (
        <div
            className={cn(
                "relative w-full overflow-hidden bg-black py-10",
                className
            )}
            style={{
